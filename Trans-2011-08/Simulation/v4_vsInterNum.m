...
XIn=[XInCenter,XInCenter+0.02+0.02j,XInCenter-0.02-0.02j,XInCenter-0.02+0.02j,XInCenter+0.02-0.02j,XInCenter+0.01+0.03j,XInCenter-0.01+0.02j,XInCenter-0.03+0.01j];
...
Ts=0.1;
TotalTestNum=3;     % number of  test loop, for averaging the outage probability
Framelength=4000;
MaxFramelength=4^3*Framelength;
...
TestPointNum=7;
 ...
 ProbVsNodeNum_CS=ones(TestPointNum,1);
 ProbVsNodeNum_OS=ones(TestPointNum,1);
 ProbVsNodeNum_SS=ones(TestPointNum,1);
 ProbVsNodeNum_OSJ=ones(TestPointNum,1);
 ProbVsNodeNum_SSJ=ones(TestPointNum,1);
 ProbVsNodeNum_Asymp_OSJ=ones(TestPointNum,1);
 ProbVsNodeNum_Asymp_SSJ=ones(TestPointNum,1);
...
for m=0:(TestPointNum-1)     
    ...
    for testloop=1:TotalTestNum   % several tests for averaging.
        ...
        if (ErrNum<30) && Framelength<MaxFramelength         % if the framelength is not long enough to have sufficient bit errors, double the framelength.
...
%               
    ...
    %%%%%%%%%%%%%%
    ProbVsNodeNum_CS(m+1)=temp_Prop_CS/TotalTestNum;
    if ProbVsNodeNum_CS(m+1)<1e-6
        ProbVsNodeNum_CS(m+1)=1e-6;
    end
    
    ProbVsNodeNum_OS(m+1)=temp_Prop_OS/TotalTestNum;
    if ProbVsNodeNum_OS(m+1)<1e-6
        ProbVsNodeNum_OS(m+1)=1e-6;
    end
     ProbVsNodeNum_SS(m+1)=temp_Prop_SS/TotalTestNum;
    if ProbVsNodeNum_SS(m+1)<1e-6
        ProbVsNodeNum_SS(m+1)=1e-6;
    end
    
    ProbVsNodeNum_OSJ(m+1)=temp_Prop_OSJ/TotalTestNum;
    if ProbVsNodeNum_OSJ(m+1)<1e-6
        ProbVsNodeNum_OSJ(m+1)=1e-6;
    end
    
    
    ProbVsNodeNum_SSJ(m+1)=temp_Prop_SSJ/TotalTestNum;
    if ProbVsNodeNum_SSJ(m+1)<1e-6
        ProbVsNodeNum_SSJ(m+1)=1e-6;
    end
    ...
    Ro=2^(2*Ts);
    ProbVsNodeNum_Asymp_OSJ(m+1)=(1-1/((SigmaEIn(1)/SigmaS1In(1)+SigmaEIn(1)/SigmaS2In(1))*Ro+1))^InterNodeNum;
    ...
    ProbVsNodeNum_Asymp_SSJ(m+1)=1+factor1;
...
figure(2);
semilogy(x,ProbVsNodeNum_CS,'y->');

hold on;
semilogy(x,ProbVsNodeNum_OS,'m-<');

hold on;
semilogy(x,ProbVsNodeNum_SS,'m-<');

hold on;
semilogy(x,ProbVsNodeNum_OSJ,'b-^');

hold on;
semilogy(x,ProbVsNodeNum_SSJ,'r-d');

hold on;
semilogy(x,ProbVsNodeNum_Asymp_OSJ,'g-d');

hold on;
semilogy(x,ProbVsNodeNum_Asymp_SSJ,'b-d');
