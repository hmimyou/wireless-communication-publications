...
%  PropVsDist_CS=ones(TestPointNum,1);
%  PropVsDist_OS=ones(TestPointNum,1);
%  PropVsDist_SS=ones(TestPointNum,1);
 PropVsDist_OSJ=ones(TestPointNum,1);
 PropVsDist_SSJ=ones(TestPointNum,1);
 PropVsDist_Asymp_OSJ=ones(TestPointNum,1);
 PropVsDist_Asymp_SSJ=ones(TestPointNum,1);
PropVsDist_min_OSJ=ones(TestPointNum,1);
 PropVsDist_min_SSJ=ones(TestPointNum,1);
 
 x=zeros(TestPointNum,1);
...
for m=0:(TestPointNum-1)     % totally TestPointNum locations  
    
    Ts=1-m/(TestPointNum-1);
...
%     temp_Prop_SS=0;
    ...
    for testloop=1:TotalTestNum   % several tests for averaging.
        ...
        if (ErrNum<40) && Framelength<MaxFramelength         % if the framelength is not long enough to have sufficient bit errors, double the framelength.
...
%         temp_Prop_SS=temp_Prop_SS+outageCount_SS/Framelength;
    ...
    %%%%%%%%%%%%%%
%     PropVsDist_CS(m+1)=temp_Prop_CS/TotalTestNum;
%     if PropVsDist_CS(m+1)<1e-6
%         PropVsDist_CS(m+1)=1e-6;
%     end
%     
%     PropVsDist_OS(m+1)=temp_Prop_OS/TotalTestNum;
%     if PropVsDist_OS(m+1)<1e-6
%         PropVsDist_OS(m+1)=1e-6;
%     end
%     
%     PropVsDist_SS(m+1)=temp_Prop_SS/TotalTestNum;
%     if PropVsDist_SS(m+1)<1e-6
%         PropVsDist_SS(m+1)=1e-6;
%     end
    
    PropVsDist_OSJ(TestPointNum-m)=temp_Prop_OSJ/TotalTestNum;
    if PropVsDist_OSJ(TestPointNum-m)<1e-6
        PropVsDist_OSJ(TestPointNum-m)=1e-6;
    end
    
    
    PropVsDist_SSJ(TestPointNum-m)=temp_Prop_SSJ/TotalTestNum;
    if PropVsDist_SSJ(TestPointNum-m)<1e-6
        PropVsDist_SSJ(TestPointNum-m)=1e-6;
    end
    ...
    Ro=2^(2*Ts);    
    PropVsDist_Asymp_OSJ(TestPointNum-m)=(1-1/((SigmaEIn(1)/SigmaS1In(1)+SigmaEIn(1)/SigmaS2In(1))*Ro+1))^InterNodeNum;
    ...
    PropVsDist_Asymp_SSJ(TestPointNum-m)=1+factor1;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 ...
 PropVsDist_min_OSJ=PropVsDist_min_OSJ.*((1-1/((SigmaEIn(1)/SigmaS1In(1)+SigmaEIn(1)/SigmaS2In(1))+1))^InterNodeNum);
...
factor1=0;
    for n=1:InterNodeNum
        C=factorial(InterNodeNum)/(factorial(n)*factorial(InterNodeNum-n));
        factor1=factor1+C*(-1)^n/(n*(SigmaEIn(1)/SigmaS1In(1)+SigmaEIn(1)/SigmaS2In(1))+1);
    end
    
    PropVsDist_min_SSJ=PropVsDist_min_SSJ.*(1+factor1);
...
figure(2);
% semilogy(x,PropVsDist_CS,'y->');
% 
% hold on;
% semilogy(x,PropVsDist_OS,'m-<');
% 
% hold on;
% semilogy(x,PropVsDist_SS,'m-<');
% 
% hold on;
semilogy(x,PropVsDist_OSJ,'b-^');

hold on;
semilogy(x,PropVsDist_SSJ,'r-d');

hold on;
semilogy(x,PropVsDist_Asymp_OSJ,'g-d');

hold on;
semilogy(x,PropVsDist_Asymp_SSJ,'b-d');

hold on;
semilogy(x,PropVsDist_min_OSJ,'y->');

hold on;
semilogy(x,PropVsDist_min_SSJ,'m-<');
