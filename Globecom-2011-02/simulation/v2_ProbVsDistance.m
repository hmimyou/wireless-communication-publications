...
% PropVsDist_SSJ=ones(TestPointNum,1);
 PropVsDist_Asymp=ones(TestPointNum,1);
...
for m=0:(TestPointNum-1)     % totally TestPointNum locations  
    ...
    XE=0.5+((1/TestStep-TestPointNum+1)*TestStep+m*TestStep)*1j;
...
%     end
    ...
    Ro=2^(2*Ts);    
    PropVsDist_Asymp(m+1)=(1-1/((SigmaEIn(1)/SigmaS1In(1)+SigmaEIn(1)/SigmaS2In(1))*Ro+1))^InterNodeNum;
...
hold on;
semilogy(x,PropVsDist_Asymp,'g-d');
