...
for m=0:(TestPointNum-1)     % totally TestPointNum locations  
    ...
    XE=0.5+((2/TestStep-TestPointNum+1)*TestStep+m*TestStep)*1j;
    ...
    x(m+1)=(1/TestStep-TestPointNum+1)*TestStep+m*TestStep;
    ...
    for testloop=1:TotalTestNum   % several tests for averaging.
        ...
        if (ErrNum<20||m>4) && Framelength<MaxFramelength         % if the framelength is not long enough to have sufficient bit errors, double the framelength.
    ...
    PropVsDist_OSJ(m+1)=temp_Prop_OSJ/TotalTestNum;
    if PropVsDist_OSJ(m+1)<1e-6
        PropVsDist_OSJ(m+1)=1e-6;
    end
    
    
    PropVsDist_SSJ(m+1)=temp_Prop_SSJ/TotalTestNum;
    if PropVsDist_SSJ(m+1)<1e-6
        PropVsDist_SSJ(m+1)=1e-6;
    end
    ...
    Ro=2^(2*Ts);    
    PropVsDist_Asymp_OSJ(m+1)=(1-1/((SigmaEIn(1)/SigmaS1In(1)+SigmaEIn(1)/SigmaS2In(1))*Ro+1))^InterNodeNum;
    ...
    PropVsDist_Asymp_SSJ(m+1)=1+factor1;
