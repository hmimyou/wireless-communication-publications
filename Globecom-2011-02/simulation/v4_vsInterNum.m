...
% ProbVsNodeNum_SSJ=ones(11,1);
 ProbVsNodeNum_Asymp=ones(TestPointNum,1);
...
for m=0:(TestPointNum-1)     % totally 10 power point  
...
%     end
    ...
    Ro=2^(2*Ts);
    ProbVsNodeNum_Asymp(m+1)=(1-1/((SigmaEIn(1)/SigmaS1In(1)+SigmaEIn(1)/SigmaS2In(1))*Ro+1))^InterNodeNum;
...
hold on;
semilogy(x,ProbVsNodeNum_Asymp,'g-d');
